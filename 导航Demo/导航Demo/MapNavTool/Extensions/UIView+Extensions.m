...
- (void)setLineSpacing:(NSInteger)lineSpacing{
    ...
    NSDictionary *dic = @{
        ...
        //以下可以按需求添加啦，还有其余的可以自己添加
//        NSFontAttributeName:[UIFont systemFontOfSize:16.0]
    };
    
    [attributedString setAttributes:dic range:NSMakeRange(0, [self.text length])];
    
    [self setAttributedText:attributedString];
    
    // 如果遇到汉语字符串中，带有英文字母或者数字的时候，会自动折行，这种情况下，只需要设置label的一个属性就好啦：label.lineBreakMode = NSLineBreakByCharWrapping;
 ...
 * lableSpace  字间距
 */
- (void)setLineSpacing:(NSInteger)lineSpacing lableSpacing:(NSInteger)lableSpacing{
    
    if (lineSpacing == 0 || lableSpacing == 0) {
        return;
    }
    ...
    if (lableSpacing) {
        ...
        NSDictionary *dic = @{NSFontAttributeName:self.font, NSParagraphStyleAttributeName:paragraphStyle, NSKernAttributeName:@(lableSpacing)};
 ...
 * lineSpacing 字间距
 */
-(void)setLabelSpacing:(NSInteger)Spacing{
    ...
    //设置字间距 NSKernAttributeName:@1.5f
    NSDictionary *dic = @{NSFontAttributeName:self.font, NSParagraphStyleAttributeName:paraStyle, NSKernAttributeName:@(Spacing)
                          };
